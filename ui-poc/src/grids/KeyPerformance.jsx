...
const KeyPerformance = () => {
    ...
    return (
        <Wrapper loading={loading} error={error} skeletonHeight={"245px"} skeletonTitle={"Loading Key Performance Indicators"}>
            <Grid container spacing={{ xs: 1, md: 1 }} columns={{ xs: 2, sm: 8, md: 12 }} sx={{ border: "1px solid #d9d9d9", padding: "5px", borderRadius: "10px", backgroundColor: "#f7f7f7", display: "flex" }}>
                <Grid size={{ xs: 1, sm: 8, md: 3 }} sx={{ display: "flex", flexDirection: "column", margin: "auto" }}>
                    ...
                    <Box sx={{ background: "white", padding: "10px", border: "1px solid #d9d9d9", borderRadius: "8px", margin: "auto" }}>
                        <ProgressCircular type={2} percentage={data?.[0]?.oee} category={"OEE"} />
                    </Box>
                </Grid>
                <Grid size={{ xs: 4, sm: 8, md: 9 }} sx={{ display: "flex", flexDirection: "column" }}>
                    <SmallProgressGrid data={data?.[0]} />
                    <Grid container spacing={{ xs: 1, md: 1 }} columns={{ xs: 4, sm: 8, md: 12 }} sx={{ marginTop: "0px", padding: "0px 20px" }}>
                        <Grid size={{ xs: 4, sm: 8, md: 12 }} sx={{ display: "flex", flexDirection: "column" }}>
                            ...
                            <ThreeStageLinearProgress actual={data?.[0]?.actual_quantity} expected={data?.[0]?.expected_quantity} target={data?.[0]?.target_quantity} total={data?.[0]?.total_quantity} />
                        </Grid>
                        <QuantityGridItem title={'Actual Quantity'} quantity={data?.[0]?.actual_quantity} />
                        <QuantityGridItem title={'Expected Quantity'} quantity={data?.[0]?.expected_quantity} />
                        <QuantityGridItem title={'Target Quantity'} quantity={data?.[0]?.target_quantity} />
                    </Grid>
                </Grid>
            </Grid>
        </Wrapper>
